...
function enhance(){
  ...
  root.querySelectorAll('.s2card').forEach(card=>{
    ...
    const range=card.querySelector('input.s2range[type="range"]');if(!range||range.dataset.tdrSensitivityEnhanced==='1')return;
    ...
    const val=card.querySelector('.s2val');if(val&&val.textContent!==`${Math.round(safe*100)}%`)val.textContent=`${Math.round(safe*100)}%`;
    ...
    range.addEventListener('input',()=>{const v=clamp(Number(range.value)||1,.5,1.5);range.value=String(v);const text=`${Math.round(v*100)}%`;if(val&&val.textContent!==text)val.textContent=text;persistSensitivity(v);});
  });
}
