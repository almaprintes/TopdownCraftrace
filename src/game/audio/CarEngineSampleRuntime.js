...
const SAMPLE_CARS=new Set(['helix_spark','helix_vortex']);
...
export class CarEngineSampleRuntime{
 ...
 async _buildGraph(){
  ...
  if(mode==='spark-samples'){
   ...
   for(const buffer of buffers){const src=ctx.createBufferSource(),gain=ctx.createGain();src.buffer=buffer;src.loop=true;gain.gain.value=0;src.connect(gain).connect(engineBus);src.start();sampleSources.push(src);sampleGains.push(gain);}
 ...
 startEngine(){this.engineStarted=true;this.unlocked=true;try{if(!this._ctx){const AC=window.AudioContext||window.webkitAudioContext;if(!AC)return;this._ctx=new AC({latencyHint:'interactive'});}if(this._ctx.state==='suspended')this._ctx.resume();this._rpm=IDLE_RPM;this._playStarter();if(!this._nodes&&!this._graphPromise){this._graphPromise=this._buildGraph().then(()=>{this._graphPromise=null;this.update(true);}).catch(e=>{this._graphPromise=null;console.warn('[TDR2 engine] sample/procedural init failed',e);if(SAMPLE_CARS.has(String(this.scene?.carId||'')))this._buildFallbackSampleGraph();});}else this.update(true);}catch(e){console.warn('[TDR2 engine] init failed',e);}}
 ...
 _targetRpm(kmh,throttle){const speedProgress=clamp(kmh/195,0,1),roadCarry=IDLE_RPM+Math.pow(speedProgress,.86)*3450;if(throttle>.05)return REDLINE_RPM;return clamp(roadCarry,IDLE_RPM,REDLINE_RPM);}
 update(force=false){
  ...
  let risePerSecond=2250;if(throttle>.05)risePerSecond=this._rpm<3300?5900:this._rpm<5600?1250:900;const fallPerSecond=2850,maxStep=(this._rpm<target?risePerSecond:fallPerSecond)*(elapsed/1000);if(this._rpm<target)this._rpm=Math.min(target,this._rpm+maxStep);else this._rpm=Math.max(target,this._rpm-maxStep);
  ...
  if(n.mode==='spark-samples'){
   ...
   const src=n.sampleSources[0],gain=n.sampleGains[0];src?.playbackRate?.setTargetAtTime?.(1.10+rpm01*1.10,now,.055);gain?.gain?.setTargetAtTime?.(.70+load*.22-coast*.08,now,.055);
  }else{
  ...
  n.cabin.frequency.setTargetAtTime(360+rpm01*420,now,.12);n.cabin.gain.setTargetAtTime(2.8-rpm01*.9+load*.45,now,.12);n.roof.frequency.setTargetAtTime(2500+rpm01*3900+load*550,now,.10);n.windFilter.frequency.setTargetAtTime(1120+speed01*2450,now,.14);n.windGain.gain.setTargetAtTime(Math.pow(speed01,1.8)*.010*p.effects,now,.12);
