...
export class CarEngineSampleRuntime{
 ...
 startEngine(){this.engineStarted=true;this.unlocked=true;try{if(!this._ctx){const AC=window.AudioContext||window.webkitAudioContext;if(!AC)return;this._ctx=new AC({latencyHint:'interactive'});}if(this._ctx.state==='suspended')this._ctx.resume();this._rpm=IDLE_RPM;this._playStarter();if(!this._nodes&&!this._graphPromise){this._graphPromise=this._buildGraph().then(()=>{this._graphPromise=null;this.update(true);}).catch(e=>{this._graphPromise=null;console.warn('[TDR2 engine] sample/procedural init failed',e);if(SAMPLE_CARS.has(String(this.scene?.carId||''))){console.error('[TDR2 engine] Spark samples unavailable; keeping engine silent rather than masking failure with synthetic fallback');}});}else this.update(true);}catch(e){console.warn('[TDR2 engine] init failed',e);}}
