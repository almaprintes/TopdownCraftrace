...
export class CarEngineSampleRuntime{
 ...
 startEngine(){this.engineStarted=true;this.unlocked=true;try{if(!this._ctx){const AC=window.AudioContext||window.webkitAudioContext;if(!AC)return;this._ctx=new AC({latencyHint:'interactive'});}if(this._ctx.state==='suspended')this._ctx.resume();this._rpm=IDLE_RPM;this._playStarter();if(!this._nodes&&!this._graphPromise){this._graphPromise=this._buildGraph().then(()=>{this._graphPromise=null;this.update(true);}).catch(e=>{this._graphPromise=null;console.warn('[TDR2 engine] sample/procedural init failed',e);});}else this.update(true);}catch(e){console.warn('[TDR2 engine] init failed',e);}}
