...
export class CarEngineSampleRuntime{
 ...
 update(force=false){
  ...
  if(this._mode()==='spark-samples'&&this._mediaSamples.length){const bodyObj=this.scene.carBody?.body,speedPx=Math.hypot(Number(bodyObj?.velocity?.x||0),Number(bodyObj?.velocity?.y||0)),kmh=Math.max(0,pxpsToKmh(speedPx)),throttle=clamp(Number(this.scene.touch?.throttle||0),0,1),p=prefs(),target=this._targetRpm(kmh,throttle),step=(this._rpm<target?2600+throttle*3300:3100)*(elapsed/1000);this._rpm=this._rpm<target?Math.min(target,this._rpm+step):Math.max(target,this._rpm-step);const rpm01=clamp((this._rpm-IDLE_RPM)/(REDLINE_RPM-IDLE_RPM),0,1),pos=rpm01*(this._mediaSamples.length-1),base=Math.floor(pos),frac=pos-base,master=p.mute?0:p.master*p.engine*.82;this._mediaSamples.forEach((a,i)=>{let level=0;if(i===base)level=1-frac;if(i===Math.min(base+1,this._mediaSamples.length-1))level=Math.max(level,frac);a.muted=false;a.playbackRate=clamp(.82+rpm01*.72,.82,1.54);a.volume=clamp(level*(.72+throttle*.18)*master,0,1);if(a.paused){const play=a.play();if(play?.catch)play.catch(()=>{});}});return;}
