...
const EVENT_DEFS=[
  ...
  {id:'clean-start',title:L('Conduce limpio','Drive Clean'),description:L('Completa 2 vueltas sin salirte del circuito.','Complete 2 laps without leaving the track.'),objective:{type:'clean',target:2,label:L('LIMPIAS','CLEAN LAPS')},reward:{items:{gear:4}}},
  ...
  {id:'clean-rhythm',title:L('Coge el ritmo','Find Your Rhythm'),description:L('Completa 5 vueltas sin abandonar la pista.','Complete 5 laps without leaving the track.'),objective:{type:'clean',target:5,label:L('LIMPIAS','CLEAN LAPS')},reward:{coins:1250}},
...
function localizeEvent(def){const k=lang();return {...def,title:def.title?.[k]||def.title?.es||def.id,description:def.description?.[k]||def.description?.es||'',objective:{...def.objective,label:def.objective?.label?.[k]||def.objective?.label?.es||''}};}
...
export function raceEventRewardLabel(reward){if(reward?.car?.id)return String(reward.car?.name?.[lang()]||reward.car?.name?.es||reward.car.id);const parts=[],coins=Math.max(0,Number(reward?.coins)||0);if(coins)parts.push(`${coins} ${t('events.coins')}`);const items=Object.entries(reward?.items||{}).filter(([,n])=>Number(n)>0);for(const [id,n] of items)parts.push(`${t(`items.${id}`)} ×${Number(n)}`);return parts.join(' · ');}
