...
#${ROOT_ID} .recipe{min-height:0;display:grid;grid-template-columns:36% minmax(0,64%);gap:7px;border:1px solid #50708c;background:#081424;border-radius:10px;padding:6px}.partCol{min-height:0;display:grid;grid-template-rows:minmax(0,1fr) 28px;gap:5px}.partImg{min-height:0;display:flex;align-items:center;justify-content:center;background:#0d1928;border:1px solid #314e68;border-radius:8px;overflow:hidden}.partImg img{width:92%;height:92%;object-fit:contain}.craftBtn{border-radius:6px;background:#273247;border:2px solid #526077;font-size:8px;font-weight:950}.craftBtn.ready{background:#17683f;border-color:#55f29b}.requirements{min-width:0;min-height:0;display:grid;grid-template-rows:auto minmax(0,1fr);gap:5px}.partName{font-size:12px;font-weight:950;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.reqGrid{min-height:0;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));grid-auto-rows:minmax(0,1fr);gap:5px}.req{position:relative;min-width:0;min-height:0;border-radius:7px;background:#101722;border:1px solid var(--tone,#ff5965);overflow:hidden;text-align:center;display:grid;grid-template-columns:34% minmax(0,66%);grid-template-rows:repeat(3,1fr);padding:4px 5px}.reqArt{grid-row:1/4;display:flex;align-items:center;justify-content:center;min-width:0;min-height:0}.reqArt img{max-width:92%;max-height:72px;object-fit:contain}.reqName{align-self:end;font-size:7px;font-weight:950;overflow:hidden;text-overflow:ellipsis}.reqPct{align-self:center;font-size:10px;font-weight:950;color:var(--tone)}.reqMeta{align-self:start;display:flex;justify-content:center;gap:5px;font-size:7px;font-weight:950}.reqQty{font-size:7px;font-weight:950}.reqState{font-size:7px;font-weight:950;color:#ffd4d7}.req.ok .reqState{color:#7dffb6}.req:before{content:"";position:absolute;left:0;right:0;bottom:0;height:var(--fill,0%);background:color-mix(in srgb,var(--tone) 24%,transparent);pointer-events:none}.req>*{position:relative}
...
export function renderWorkshopMobileDom(scene){
 ...
 const info=el('div','requirements');info.append(el('div','partName',String(item?.name||out).toUpperCase()));const grid=el('div','reqGrid');info.append(grid);card.append(info);reqs.forEach(s=>{const tone=progressTone(s.p),r=el('div',`req ${s.ok?'ok':''}`);r.style.setProperty('--tone',tone);r.style.setProperty('--fill',`${Math.round(s.p*100)}%`);const ra=el('div','reqArt');if(s.item){const ri=el('img');ri.src=asset(s.item);ri.alt='';ra.append(ri);}const meta=el('div','reqMeta');meta.append(el('span','reqQty',`${s.have} / ${s.need}`),el('span','reqState',s.ok?'LISTO':`FALTAN ${Math.max(0,s.need-s.have)}`));r.append(ra,el('div','reqName',String(s.item?.name||s.req.id).toUpperCase()),el('div','reqPct',`${Math.round((s.have/s.need)*100)}%`),meta);if(!s.ok&&scene._openRecyclerForMaterial)r.addEventListener('click',()=>scene._openRecyclerForMaterial(s.req.id,{targetNeed:s.need,requirements:state.map(x=>({id:x.req.id,need:x.need}))}));grid.append(r);});
