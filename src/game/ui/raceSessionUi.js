...
export function mountRaceSessionRewards({baseUrl='/',laps=0,bonusLaps=0,entries=[],resultLabel='VER INFORME',onFinish=()=>{}}={}){
  ...
  const reveal=()=>{
    ...
    entries.forEach((_,i)=>setTimeout(()=>root.querySelector(`[data-r="${i}"]`)?.classList.add('show'),220+i*90));
    setTimeout(()=>next?.classList.add('show'),300+entries.length*90);
  };
