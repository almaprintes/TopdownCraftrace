...
function nextMonthStart(now=new Date()){return new Date(now.getFullYear(),now.getMonth()+1,1,0,0,0,0);}
function seasonCountdownLabel(){const ms=Math.max(0,nextMonthStart().getTime()-Date.now()),s=Math.ceil(ms/1000),d=Math.floor(s/86400),h=Math.floor((s%86400)/3600),m=Math.floor((s%3600)/60),sec=s%60;return `${d}D · ${pad2(h)}:${pad2(m)}:${pad2(sec)}`;}
...
function trackSvg(center) { if (!Array.isArray(center) || center.length < 2) return ''; const xs = center.map(p => Number(p?.x)).filter(Number.isFinite); const ys = center.map(p => Number(p?.y)).filter(Number.isFinite); if (!xs.length || !ys.length) return ''; const minX = Math.min(...xs), maxX = Math.max(...xs), minY = Math.min(...ys), maxY = Math.max(...ys); const spanX = Math.max(1, maxX - minX), spanY = Math.max(1, maxY - minY); const scale = Math.min(150 / spanX, 82 / spanY); const ox = 90 - spanX * scale / 2, oy = 48 - spanY * scale / 2; const points = center.map(p => `${(ox + (Number(p.x) - minX) * scale).toFixed(1)},${(oy + (Number(p.y) - minY) * scale).toFixed(1)}`).join(' '); return `<svg viewBox="0 0 180 96"><polyline class="tdr-track-shadow" points="${points}"/><polyline class="tdr-track-line" points="${points}"/></svg>`; }
...
function renderDomCards(scene, root) {
  ...
  if (eventSlot) {
    if (eventData.finished) {
      ...
      eventSlot.innerHTML = `<div class="tdr-card-kicker">${en?'SEASON 0 · COMPLETE':'TEMPORADA 0 · COMPLETADA'}</div><h2>${en?'COMPLETE DRIVER':'PILOTO COMPLETO'}</h2><p>${en?'YOU COMPLETED ALL 14 INDUCTION STAGES':'HAS COMPLETADO LAS 14 ETAPAS DE INDUCCIÓN'}</p><div class="tdr-season-next">${en?'NEXT SEASON IN':'PRÓXIMA TEMPORADA EN'}</div><div class="tdr-event-finished" data-season-countdown>${seasonCountdownLabel()}</div>`;
...
export function installLobbyDom(scene) {
  ...
  if (!root?.isConnected) {
    ...
    root.innerHTML = `<header class="tdr-lobby-header"><button type="button" class="tdr-lobby-brand" aria-label="Top Down Race"><img src="${BASE}assets/logo.webp" alt="Top Down Race" draggable="false"></button><div class="tdr-lobby-wallet"><img src="${BASE}assets/ui/moneda-tdr.webp" alt="" draggable="false"><span><small>${tr('lobby.coins')}</small><strong data-coins>0</strong></span></div><nav class="tdr-lobby-top-actions"></nav></header><img class="tdr-lobby-car-preview" data-lobby-car alt="" draggable="false"><main class="tdr-lobby-cards"><section class="tdr-lobby-card tdr-event-card" data-event-card></section><section class="tdr-lobby-card tdr-car-card" data-car-card></section><section class="tdr-lobby-card tdr-track-card" data-track-card></section></main><div class="tdr-lobby-play-slot"></div><nav class="tdr-lobby-bottom-actions"></nav>`;
  ...
  const syncCoins=()=>{const coins=Math.max(0,Math.floor(Number(loadGarage()?.coins)||0));const coinNode=root.querySelector('[data-coins]');if(coinNode)coinNode.textContent=coins.toLocaleString(getLanguage()==='es'?'es-ES':'en-US');const countdown=root.querySelector('[data-season-countdown]');if(countdown)countdown.textContent=seasonCountdownLabel();};
  syncCoins();
