...
export function installRacePerformanceProbe(RaceScene){
  ...
  proto.update=function(time,delta){
    const start=performance.now(),result=originalUpdate?.call(this,time,delta),end=performance.now();this._tdrCost.race=Math.max(0,end-start);
    try{
      const frameMs=Math.max(0,end-(this._tdrPerfLast||end));this._tdrPerfLast=end;if(frameMs>=32){this._tdrSpikeCount=(this._tdrSpikeCount||0)+1;const now=performance.now(),gap=this._tdrSpikeAt?now-this._tdrSpikeAt:0;this._tdrSpikeAt=now;this._tdrSpikeGap=gap;const flags=[];if(this._replayActive)flags.push('GHOST');if(this._sessionReportOpen)flags.push('REPORT');if(this._tdrPauseMenuOpen)flags.push('PAUSE');if(this._tdrLiveDeltaUi?.root?.isConnected)flags.push('DELTA');if(this._tdrStaticMinimap?.root?.isConnected)flags.push('MAP');if(this._tdrStaticGhostPanel?.isConnected)flags.push('GHUD');this._tdrSpikeLast=`${frameMs.toFixed(1)}ms ${flags.join('+')||'RACE'}`;}this._tdrPerfFrames=(this._tdrPerfFrames||0)+1;this._tdrPerfElapsed=(this._tdrPerfElapsed||0)+frameMs;this._tdrPerfWorst=Math.max(this._tdrPerfWorst||0,frameMs);this._tdrPerfSamples?.push(frameMs);if(frameMs>20)this._tdrPerfOver20++;if(frameMs>33)this._tdrPerfOver33++;if(frameMs>50)this._tdrPerfOver50++;if(this._tdrInputPendingAt){const lag=Math.max(0,start-this._tdrInputPendingAt);this._tdrInputLastLatency=lag;this._tdrInputMaxLatency=Math.max(this._tdrInputMaxLatency||0,lag);this._tdrInputPendingAt=0;}
