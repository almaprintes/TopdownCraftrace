...
export function installRacePerformanceProbe(RaceScene){
  ...
  proto.update=function(time,delta){
    ...
    try{
      const frameMs=Math.max(0,end-(this._tdrPerfLast||end));this._tdrPerfLast=end;if(frameMs>=32){this._tdrSpikeCount=(this._tdrSpikeCount||0)+1;if(frameMs>=(this._tdrSpikeWorst||0)){this._tdrSpikeWorst=frameMs;this._tdrSpikeLast=`${frameMs.toFixed(1)}ms U${(this._tdrPhase.update||0).toFixed(1)} P${(this._tdrPhase.post||0).toFixed(1)} O${(this._tdrPhase.other||0).toFixed(1)}`;}}this._tdrPerfFrames=(this._tdrPerfFrames||0)+1;this._tdrPerfElapsed=(this._tdrPerfElapsed||0)+frameMs;this._tdrPerfSamples?.push(frameMs);if(frameMs>33)this._tdrPerfOver33++;if(frameMs>50)this._tdrPerfOver50++;
