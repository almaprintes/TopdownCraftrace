...
export class RaceScene extends CurrentRaceScene{
  ...
  _createStatsReplayOverlay(payload){
    ...
    try{document.querySelectorAll('[data-tdr-ghost-diag]').forEach(el=>el.remove());}catch{}
    ...
    try{
      let report=null;
      const packet=encodeOnlineGhost(payload);
      const error=measureOnlineGhostError(payload,packet);
      const originalBytes=new TextEncoder().encode(JSON.stringify(payload)).byteLength;
      const compactBytes=onlineGhostJsonBytes(packet);
      report={version:'DEV 1.0.192',trackId:String(payload.trackId||payload.trackKey||this.trackKey||''),lapMs:Math.round(Number(payload.lapMs)||0),originalSamples:Array.isArray(payload.samples)?payload.samples.length:0,compactSamples:Array.isArray(packet.p)?packet.p.length:0,originalBytes,compactBytes,savingPct:originalBytes?Math.round((1-compactBytes/originalBytes)*1000)/10:0,maxPositionError:Math.round(error.maxPositionError*1000)/1000,rmsPositionError:Math.round(error.rmsPositionError*1000)/1000,maxAngleError:Math.round(error.maxAngleError*100000)/100000,probes:error.probes,createdAt:Date.now()};
      try{localStorage.setItem(GHOST_REPORT_KEY,JSON.stringify(report));}catch{}
      const diag=document.createElement('div');diag.dataset.tdrGhostDiag='1';
      diag.style.cssText='position:fixed;right:max(12px,env(safe-area-inset-right));bottom:max(12px,env(safe-area-inset-bottom));z-index:2147483646;width:min(360px,42vw);padding:8px 10px;background:rgba(3,14,22,.92);border:1px solid rgba(99,243,165,.55);font-size:9px;line-height:1.35;pointer-events:none';
      diag.innerHTML='<div style="font-size:8px;font-weight:1000;letter-spacing:.14em;color:#63f3a5">DEV 1.0.192 · ONLINE GHOST DIAGNOSTIC</div>'+
      ...
      const diag=document.createElement('div');diag.dataset.tdrGhostDiag='1';diag.style.cssText='position:fixed;right:12px;bottom:12px;padding:8px 10px;background:#240b0b;border:1px solid #ff6b6b;font-size:9px;z-index:2147483646';diag.textContent='GHOST DIAGNOSTIC ERROR: '+String(err?.message||err);document.body.appendChild(diag);
    }
