...
if(current&&tester&&!current.__tdrTesterExchangePatch){
  ...
  current._renderBroadcastRecords=function(trackId=null){
    ...
    root.appendChild(b);
    const decorate=()=>{if(!this._root?.isConnected)return;this._root.querySelectorAll('button[title="Compartir vuelta"]').forEach(el=>el.remove());this._addShareButtons?.(this._selectedTrack?.()||id);};
    decorate();
