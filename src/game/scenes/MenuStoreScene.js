...
export class MenuScene extends PreviousMenuScene{
 preload(){super.preload?.();preloadTdrCoin(this);for(const id of COIN_PACK_ASSETS){const key=`store:${id}`;if(!this.textures.exists(key))this.load.image(key,`${import.meta.env.BASE_URL||'/'}assets/store/${id}.webp`);}for(const id of ['scrap','alloy','rubber','compound','disc','spring','gear','ecu']){const it=GARAGE_ITEMS[id];if(it?.asset&&!this.textures.exists(`store:${id}`))this.load.image(`store:${id}`,it.asset);}}
