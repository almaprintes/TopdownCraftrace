...
export class UpgradeShopScene extends Phaser.Scene{
  ...
  render(){
    ...
    add(this.add.text(pad,48,`${spec.name}  ·  ${t('workshop.drag')} · ${t('workshop.fuse')} · ${t('workshop.mount')} · ${t('workshop.race')}`,{fontFamily:'system-ui',fontSize:'10px',fontStyle:'900',color:'#5dffad',letterSpacing:1.25}));
    const back=add(this.add.text(width-pad,20,`← ${t('workshop.garage')}`,{fontFamily:'system-ui',fontSize:'13px',fontStyle:'900',color:'#e5f3f7'}).setOrigin(1,0).setInteractive({useHandCursor:true}));
    ...
    panel(pad,leftW,`1 · ${t('workshop.storage')}`,0x57efa5);panel(pad+leftW+12,midW,`2 · ${t('workshop.fusionBench')}`,0x4cbcff);panel(rightX,rightW,`3 · ${t('workshop.assembly')}`,0xffc857);

    const tabY=y+46,tabW=(leftW-36)/2;
    this._tab(add,pad+14,tabY,tabW,t('workshop.materials'),'materials');this._tab(add,pad+22+tabW,tabY,tabW,t('workshop.parts'),'parts');
    add(this.add.text(pad+14,tabY+35,this.filter==='materials'?t('workshop.dragMaterials'):t('workshop.dragParts'),{fontFamily:'system-ui',fontSize:'9px',fontStyle:'800',color:'#7f99a2'}));
    ...
    add(this.add.text(mx+14,y+47,t('workshop.fusionHelp'),{fontFamily:'system-ui',fontSize:'9px',color:'#89a4ad',lineSpacing:4,wordWrap:{width:midW-28}}));
    ...
    if(out){this._drawItemIcon(add,out,mx+42,ry+27,34);add(this.add.text(mx+68,ry+12,t('workshop.result'),{fontFamily:'system-ui',fontSize:'7.5px',fontStyle:'900',color:'#d9a93f'}));add(this.add.text(mx+68,ry+27,out.name,{fontFamily:'system-ui',fontSize:'10px',fontStyle:'900',color:'#fff',wordWrap:{width:midW-102}}));}
    else add(this.add.text(mx+midW/2,ry+27,t('workshop.noRecipe'),{fontFamily:'Orbitron,system-ui',fontSize:'9px',fontStyle:'900',color:'#4e6872'}).setOrigin(.5));
    ...
    add(this.add.text(mx+midW/2,by+23,out?t('workshop.fuse'):t('workshop.dragTwoMaterials'),{fontFamily:'Orbitron,system-ui',fontSize:'9.5px',fontStyle:'900',color:out?'#041d12':'#6e858e'}).setOrigin(.5));btn.on('pointerdown',()=>this._craft());
    add(this.add.text(mx+14,by+62,t('workshop.hints'),{fontFamily:'system-ui',fontSize:'8px',fontStyle:'900',color:'#66818b'}));
    ...
    add(this.add.text(rightX+14,py+1,t('workshop.realEffect'),{fontFamily:'system-ui',fontSize:'7.5px',color:'#657b83'}));
    ...
    add(this.add.text(rightX+14,y+h-23,`${t('workshop.parts')} → ${t('workshop.dragCorrectSlot')}`,{fontFamily:'system-ui',fontSize:'8px',fontStyle:'900',color:'#c5a54b'}));
  }
  ...
  _inventoryCard(add,{id,item,q,x,y,w,h,eq,just}){
    ...
    c.add(this.add.text(w/2,h-12,eq?t('workshop.mounted'):t('workshop.drag'),{fontFamily:'system-ui',fontSize:'6.8px',fontStyle:'900',color:eq?'#61f2aa':'#607d87'}).setOrigin(.5));
  ...
  _fusionSlot(add,key,x,y,w,h,id){
    ...
    if(item){this._drawItemIcon(add,item,x+w/2,y+36,48);add(this.add.text(x+w/2,y+70,item.name,{fontFamily:'system-ui',fontSize:'8px',fontStyle:'900',color:'#e7f4f7',align:'center',wordWrap:{width:w-14}}).setOrigin(.5,0));add(this.add.text(x+w/2,y+h-10,t('workshop.tapRemove'),{fontFamily:'system-ui',fontSize:'6.5px',fontStyle:'900',color:'#6d8a94'}).setOrigin(.5));}
    else{add(this.add.text(x+w/2,y+31,key,{fontFamily:'Orbitron,system-ui',fontSize:'24px',fontStyle:'900',color:'#35525d'}).setOrigin(.5));add(this.add.text(x+w/2,y+68,t('workshop.dropHere'),{fontFamily:'system-ui',fontSize:'7px',fontStyle:'900',color:'#4e6c76'}).setOrigin(.5));}
  ...
  _mountSlot(add,f,x,y,w,h,item){
    ...
    add(this.add.text(x+10,y+7,familyLabel(f),{fontFamily:'system-ui',fontSize:'7.5px',fontStyle:'900',color:'#728991'}));
    ...
    else add(this.add.text(x+14,y+28,t('workshop.dropHere'),{fontFamily:'system-ui',fontSize:'8px',fontStyle:'900',color:'#49636d'}));
  }

  _handleDrop(id,source,pointer){
    const item=GARAGE_ITEMS[id];if(!item){this._dragging=false;return this.render();}
    if(item.kind==='part'){
      const z=this._dropZones[`family:${item.family}`];if(inside(pointer,z)&&equip(this.state,id,this.carId)){this.state=loadGarage();this._toast(`${item.name} montada en ${familyLabel(item.family)}`);}
    }else{
