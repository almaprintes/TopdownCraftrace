...
export class UpgradeShopScene extends Phaser.Scene{
  ...
  _header(A,w,compact){
    ...
    A(this.add.text(compact?51:61,h/2,t('workshop.garage'),{fontFamily:'Arial Narrow,system-ui',fontSize:compact?'18px':'23px',fontStyle:'900 italic',color:'#fff'}).setOrigin(0,.5));
    A(this.add.text(w*.58,h/2,t('workshop.craftStrip'),{fontFamily:'Arial Narrow,system-ui',fontSize:compact?'20px':'27px',fontStyle:'900 italic',color:'#fff'}).setOrigin(.5));
  ...
  _carPanel(A,r,compact){
    ...
    const y=r.y+(compact?10:14);A(this.add.text(r.x+15,y,t('workshop.currentCar'),{fontFamily:'system-ui',fontSize:compact?'10px':'12px',fontStyle:'800',color:'#8da3ae'}));
    ...
    A(this.add.text(r.x+r.w-15,y+2,String(spec.rarity||t('workshop.rarity.common')).toUpperCase(),{fontFamily:'system-ui',fontSize:compact?'10px':'12px',fontStyle:'900',color:'#ffd05a'}).setOrigin(1,0));
  ...
  _craftPanel(A,r,compact){
    ...
    const ty=ry+rh+7,th=compact?27:33;this._tab(A,r.x+14,ty,(r.w-34)/2,th,t('workshop.materials'),'materials',compact);this._tab(A,r.x+20+(r.w-34)/2,ty,(r.w-34)/2,th,t('workshop.parts'),'parts',compact);
  ...
  _result(A,r,compact){
    ...
    if(!item){A(this.add.text(r.x+r.w/2,r.y+r.h/2,this.slots.length?t('workshop.completeRecipe'):t('workshop.selectThreeComponents'),{fontFamily:'Arial Narrow,system-ui',fontSize:compact?'12px':'15px',fontStyle:'900',color:'#718993'}).setOrigin(.5));return;}
    this._itemArt(A,item,r.x+r.w*.18,r.y+r.h*.46,Math.min(r.h*.62,r.w*.18));A(this.add.text(r.x+r.w*.31,r.y+12,item.name.toUpperCase(),{fontFamily:'Arial Narrow,system-ui',fontSize:compact?'16px':'21px',fontStyle:'900 italic',color:'#dc59ff'}));A(this.add.text(r.x+r.w*.31,r.y+(compact?35:43),`${familyLabel(item.family)||t('workshop.part')} · ${t('workshop.level',{level:item.tier||1})}`,{fontFamily:'system-ui',fontSize:compact?'9px':'11px',fontStyle:'800',color:'#dac2df'}));
    const spec=CAR_SPECS[this.car]||CAR_SPECS.stock,cur=garageDisplayStats(spec,this.state,this.car,null),next=garageDisplayStats(spec,this.state,this.car,item.id),changes=statRows().filter(([k])=>next[k]!==cur[k]).map(([k,l])=>`${l} ${cur[k]} › ${next[k]}`);
    A(this.add.text(r.x+r.w*.31,r.y+(compact?53:65),changes.slice(0,2).join('   ·   ')||t('workshop.readyToEquip'),{fontFamily:'Arial Narrow,system-ui',fontSize:compact?'10px':'12px',fontStyle:'900',color:'#65ef70'}));
    const btn=A(this.add.rectangle(r.x+r.w*.73,r.y+r.h-(compact?18:23),r.w*.45,compact?29:37,0xf5bb11).setStrokeStyle(1,0xffdf58).setInteractive({useHandCursor:true}));A(this.add.text(btn.x,btn.y,t('workshop.craft'),{fontFamily:'Arial Narrow,system-ui',fontSize:compact?'14px':'18px',fontStyle:'900',color:'#111'}).setOrigin(.5));btn.on('pointerdown',()=>this._craft());
  }
  ...
  _inventory(A,r,compact){
    ...
    if(!ids.length)A(this.add.text(r.x+r.w/2,r.y+r.h/2,this.slots.length===3?t('workshop.recipeComplete'):t('workshop.noCompatibleComponents'),{fontFamily:'Arial Narrow,system-ui',fontSize:compact?'10px':'12px',fontStyle:'900',color:'#627985'}).setOrigin(.5));
  }
  ...
  _equippedStrip(A,r,compact){
    ...
    FAMILIES.forEach((f,i)=>{const x=r.x+i*(cw+gap),item=eq[f]?GARAGE_ITEMS[eq[f]]:null,q=A(this.add.graphics());q.fillStyle(item?0x0a1b21:0x081116);q.fillRoundedRect(x+2,r.y+2,cw-4,r.h-4,6);q.lineStyle(1,item?(item.tone||0x2dcfff):0x2b424c,.65);q.strokeRoundedRect(x+2,r.y+2,cw-4,r.h-4,6);A(this.add.text(x+cw/2,r.y+(compact?14:17),familyLabel(f),{fontFamily:'Arial Narrow,system-ui',fontSize:compact?'9px':'11px',fontStyle:'900',color:'#fff'}).setOrigin(.5));A(this.add.text(x+cw/2,r.y+r.h-(compact?13:16),item?`${item.name} · T${item.tier}`:t('workshop.unequipped'),{fontFamily:'system-ui',fontSize:compact?'8px':'9px',fontStyle:'700',color:item?'#66dfff':'#667a83'}).setOrigin(.5));});
  }
  ...
  _craft(){if(this.busy)return;const recipe=findStripRecipe(this.slots);if(!recipe)return this._toast(t('workshop.incompleteCombination'));this.busy=true;const res=craftStrip(this.state,this.slots);if(!res.ok){this.busy=false;return this._toast(res.reason||t('workshop.cannotCraft'));}this.state=loadGarage();if(res.item?.kind==='part')equip(this.state,res.item.id,this.car);this.state=loadGarage();this.slots=[];this.cameras.main.shake(90,.0012);this.busy=false;this._toast(t('workshop.craftedAndEquipped',{part:res.item?.name||t('workshop.part')}));this.render();}
