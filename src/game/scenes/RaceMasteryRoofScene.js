...
export class RaceScene extends CurrentRaceScene{
  create(data){
    ...
    this.time?.delayedCall?.(80,()=>this._installMasteryRoofWheel());
    return result;
  }
  ...
  _installMasteryRoofWheel(){
    ...
    if(!mastery.level)return;
    try{this._masteryRoofWheel?.destroy?.();}catch{}
    const sprite=visibleCarSprite(this);
    const carW=Math.max(18,Number(sprite?.displayWidth||sprite?.width||28));
    const radius=Math.max(4.5,Math.min(8,carW*.19));
    ...
    g.setDepth?.(999);
    rig.add(g);this._masteryRoofWheel=g;
  }
}
