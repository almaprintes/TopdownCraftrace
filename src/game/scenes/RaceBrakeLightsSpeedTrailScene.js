...
export class RaceScene extends CurrentRaceScene {
  ...
  _visualCar(){
    return this.carBody?.scene ? this.carBody : this.car;
  }

  _buildBrakeLights(){
    const car=this._visualCar();
    if(!car?.scene)return;
    const make=()=>this.add.circle(0,0,3.2,0xff2518,0)
      .setDepth(Number(car.depth||20)+1)
      .setScrollFactor(1)
      .setBlendMode('ADD');
  ...
  _updateBrakeLights(){
    const car=this._visualCar();
    if(!car?.scene || this._brakeLights.length!==2)return;
    const rot=Number(car.rotation||0);
    const fx=Math.cos(rot), fy=Math.sin(rot), lx=-fy, ly=fx;
    const rear=25, half=12.5;
    const cx=Number(car.x||0)-fx*rear, cy=Number(car.y||0)-fy*rear;
    const braking=this._brakePressed() && !!this._raceStarted;
    const alpha=braking?0.92:0.08;
    const radius=braking?3.6:2.6;
    const left=this._brakeLights[0], right=this._brakeLights[1];
    left.setPosition(cx+lx*half,cy+ly*half).setRadius(radius).setAlpha(alpha);
    right.setPosition(cx-lx*half,cy-ly*half).setRadius(radius).setAlpha(alpha);
  }
  ...
  _updateSpeedTrail(now){
    ...
    const car=this._visualCar();
    const body=car?.body;
    if(!car?.scene || !body?.velocity)return;
    ...
    const key=car.texture?.key;
    if(!key || !this.textures.exists(key))return;
    const ghost=this.add.image(car.x,car.y,key)
      .setRotation(car.rotation)
      .setOrigin(car.originX??0.5,car.originY??0.5)
      .setScale(car.scaleX||1,car.scaleY||1)
      .setDepth(Number(car.depth||20)-0.2)
      .setAlpha(0.10)
      .setTint(0xdcecff)
      .setScrollFactor(1);
    ...
    this.tweens.add({
      targets:ghost,
      alpha:0,
      scaleX:(car.scaleX||1)*1.015,
      scaleY:(car.scaleY||1)*1.015,
