...
export class TrackGarageScene extends BaseScene {
  constructor(){
    ...
    this._tracks=[]; this._index=0; this._ui=null; this._downX=0; this._dragged=false; this._mode='player';
  }
  ...
  create(){
    ...
    this.input.on('pointerup',p=>{const dx=p.x-this._downX;if(this._dragged&&Math.abs(dx)>70)this._move(dx<0?1:-1);});
    this._render();
  }
  ...
  _render(){
    ...
    add(this.add.text(pad,top+40,this._mode==='admin'?'GESTIONA LA BIBLIOTECA DE TRAZADOS':'ELIGE TU PRÓXIMO TRAZADO',{fontFamily:'system-ui',fontSize:'11px',fontStyle:'700',color:'#62eaa1',letterSpacing:2}));
    ...
    const right=add(this.add.text(pad+previewW-16,contentTop+contentH/2,'›',{fontFamily:'system-ui',fontSize:'46px',color:'#fff',backgroundColor:'#0b1218aa',padding:{x:7,y:0}}).setOrigin(1,.5).setInteractive({useHandCursor:true}));left.on('pointerdown',()=>this._move(-1));right.on('pointerdown',()=>this._move(1));
    ...
    const badge=(x,y,text,col)=>add(this.add.text(x,y,text,{fontFamily:'system-ui',fontSize:'11px',fontStyle:'800',color:'#fff',backgroundColor:col,padding:{x:10,y:6}}));
    badge(infoX,contentTop+94,`DIFICULTAD · ${track.difficulty||'—'}`,'#263a45');badge(infoX,contentTop+128,`LONGITUD · ${track.lengthLabel||'—'}`,'#263a45');
    const len=lengthOf(track);add(this.add.text(infoX,contentTop+174,`≈ ${len.toLocaleString('es-ES')} px`,{fontFamily:'Orbitron,system-ui',fontSize:'16px',color:'#d9f6e7'}));add(this.add.text(infoX,contentTop+204,`${Math.round(track.trackWidth)} px de ancho\n${track.worldW} × ${track.worldH} mundo`,{fontFamily:'system-ui',fontSize:'12px',color:'#8fa5ad',lineSpacing:6}));
