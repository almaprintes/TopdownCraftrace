...
export class MenuScene extends CurrentMenuScene{
  renderUI(){super.renderUI();this._removeLegacyHeroHud();this._renderWelcomeCards();}
  ...
  _renderWelcomeCards(){
    ...
    const spec=CAR_SPECS?.[this.selectedCarId];
    if(!track||!spec)return;

    // One intentional top rail. Both cards share baseline, height and visual language,
    // leaving the car, event panel, mode selector and play button completely unobstructed.
    const topY=clamp(Math.floor(height*.165),102,124);
    const gap=clamp(Math.floor(width*.015),16,24);
    const rightMargin=clamp(Math.floor(width*.055),58,88);
    const trackW=clamp(Math.floor(width*.295),400,470);
    const carW=clamp(Math.floor(width*.205),286,330);
    const cardH=clamp(Math.floor(height*.135),92,104);
  ...
  _renderTrackCard(x,y,w,h,track,key){
    ...
    const previewW=104,previewH=44,previewX=left+52,previewY=top+62;
    c.add(this.add.rectangle(previewX,previewY,previewW,previewH,0x050b10,.58).setOrigin(.5).setStrokeStyle(1,0xd8a73a,.2));
    ...
    const sx=left+120,sy=top+37,cellW=Math.max(92,Math.floor((w-148)/2)),cellH=27;
    const ls={fontFamily:'system-ui,-apple-system,Segoe UI,Arial',fontSize:'7px',fontStyle:'bold',color:'#8b97a8'};
    const vs={fontFamily:'system-ui,-apple-system,Segoe UI,Arial',fontSize:'10px',fontStyle:'bold',color:'#ffffff'};
    [['LONGITUD',`${lengthM} m`],['SECTORES',String(sectors)],['SUPERFICIE',surface],['SENTIDO',direction]].forEach((r,i)=>{const col=i%2,row=Math.floor(i/2),rx=sx+col*cellW,ry=sy+row*cellH;c.add(this.add.text(rx,ry,r[0],ls).setOrigin(0));c.add(this.add.text(rx,ry+9,r[1],vs).setOrigin(0));});
  }
}
