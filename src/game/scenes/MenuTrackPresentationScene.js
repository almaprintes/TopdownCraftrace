...
function surfaceKey(track){const id=String(track?.id||track?.key||'').toLowerCase(),cat=String(track?.category||'').toLowerCase();return(id.includes('offroad')||id.includes('raven')||cat.includes('dirt')||cat.includes('tierra'))?'dirt':'asphalt';}
...
export class MenuScene extends CurrentMenuScene{
  ...
  _renderCarCard(x,y,w,h,spec){
    ...
    const left=-w/2+10,top=-h/2+5;c.add(this.add.text(left,top,t('menuPresentation.selectedCar'),{fontFamily:'system-ui,-apple-system,Segoe UI,Arial',fontSize:'7px',fontStyle:'bold',color:'#6deaff',letterSpacing:1}).setOrigin(0));
  ...
  _renderTrackCard(x,y,w,h,track,key){
    ...
    c.add(this.add.text(0,top+12,t('menuPresentation.selectedTrack'),{fontFamily:'system-ui,-apple-system,Segoe UI,Arial',fontSize:'9px',fontStyle:'bold',color:'#f0c65a',letterSpacing:1,align:'center'}).setOrigin(.5,0));
    ...
    const lengthM=Math.round(loopLength(center)*METERS_PER_PX),sectors=Math.max(1,(track.checkpoints?.length||2)+1),surface=t(`menuPresentation.surface.${surfaceKey(track)}`),direction=t(String(track.raceDirection||'forward').toLowerCase()==='reverse'?'menuPresentation.direction.counterclockwise':'menuPresentation.direction.clockwise');
    const stats=[[t('menuPresentation.length'),`${lengthM} m`],[t('menuPresentation.sectors'),String(sectors)],[t('menuPresentation.surface'),surface],[t('menuPresentation.direction'),direction]];
  ...
  _renderGlobalEventCard(){
    ...
    c.add(this.add.text(0,top+12,complete?t('menuPresentation.eventComplete'):t('menuPresentation.globalEvent'),{fontFamily:'system-ui,-apple-system,Segoe UI,Arial',fontSize:'9px',fontStyle:'bold',color:accent,letterSpacing:1,align:'center'}).setOrigin(.5,0));
    c.add(this.add.text(0,top+38,complete?t('menuPresentation.twentyLapsAchieved'):t('menuPresentation.twentyLapChallenge'),{fontFamily:'system-ui,-apple-system,Segoe UI,Arial',fontSize:'17px',fontStyle:'bold',color:'#fff',align:'center',lineSpacing:1}).setOrigin(.5,0));
    c.add(this.add.text(0,top+88,t('menuPresentation.addLapsAnywhere'),{fontFamily:'system-ui,-apple-system,Segoe UI,Arial',fontSize:'8px',fontStyle:'bold',color:'#aebdca',align:'center',lineSpacing:1,wordWrap:{width:w-26}}).setOrigin(.5,0));
    if(best!=null)c.add(this.add.text(0,top+118,t('menuPresentation.globalBest',{time:this._formatMissionTime(best)}),{fontFamily:'system-ui,-apple-system,Segoe UI,Arial',fontSize:'8px',fontStyle:'bold',color:'#93a5b8',align:'center'}).setOrigin(.5,0));
    ...
    c.add(this.add.text(0,bottom-22,t('menuPresentation.lapProgress',{done,target}),{fontFamily:'system-ui,-apple-system,Segoe UI,Arial',fontSize:'10px',fontStyle:'bold',color:accent,align:'center'}).setOrigin(.5,0));
  }
