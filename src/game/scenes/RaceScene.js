...
import { loadGarage, garageTuning, grantRaceReward } from '../garage/garageStore.js';
...
if (!this._useFactorySpec) {
  ...
  // Tuning derivado desde upgrades legacy + Garage Fusion.
  // Garage Fusion es acumulativo y no destruye las mejoras antiguas ya guardadas.
  this.tuningBase = tuningFromUpgrades(this.upgrades);
  if (!this._useFactorySpec) {
    const gf = garageTuning(loadGarage());
    this.tuningBase = {
      ...this.tuningBase,
      accelMult: (this.tuningBase.accelMult || 1) * (gf.accelMult || 1),
      brakeMult: (this.tuningBase.brakeMult || 1) * (gf.brakeMult || 1),
      dragMult: (this.tuningBase.dragMult || 1) * (gf.dragMult || 1),
      turnRateMult: (this.tuningBase.turnRateMult || 1) * (gf.turnRateMult || 1),
      maxFwdAdd: (this.tuningBase.maxFwdAdd || 0) + (gf.maxFwdAdd || 0),
      maxRevAdd: (this.tuningBase.maxRevAdd || 0) + (gf.maxRevAdd || 0),
      turnMinAdd: (this.tuningBase.turnMinAdd || 0) + (gf.turnMinAdd || 0),
      gripCoastAdd: (this.tuningBase.gripCoastAdd || 0) + (gf.gripCoastAdd || 0),
      gripDriveAdd: (this.tuningBase.gripDriveAdd || 0) + (gf.gripDriveAdd || 0),
      gripBrakeAdd: (this.tuningBase.gripBrakeAdd || 0) + (gf.gripBrakeAdd || 0)
    };
  }
  // tuning final = progresión + devTuning (dev manda por encima)
...
const fl = j.finishLine || j.finish || j.__autoFinishLine || null;
  ...
  _showGarageRewardToast(reward) {
    if (!reward || !this.add) return;
    const cam = this.uiCam || this.cameras?.main;
    const w = this.scale.width;
    const h = this.scale.height;
    const text = `TALLER  +${reward.scrap || 0} chatarra  +${reward.alloy || 0} aleación  +${reward.rubber || 0} goma`;
    const bg = this.add.rectangle(w / 2, h - 34, Math.min(w - 24, 390), 34, 0x071018, 0.92)
      .setStrokeStyle(1, 0x4ee1a0, 0.8)
      .setDepth(10050);
    const tx = this.add.text(w / 2, h - 34, text, {
      fontFamily: 'system-ui, -apple-system, Segoe UI, Arial',
      fontSize: '10px', fontStyle: '700', color: '#dfffee'
    }).setOrigin(0.5).setDepth(10051);
    try { this.cameras.main.ignore([bg, tx]); } catch {}
    try { this.uiCam?.ignore?.([]); } catch {}
    this.tweens.add({ targets:[bg,tx], alpha:0, y:'-=10', delay:1800, duration:350,
