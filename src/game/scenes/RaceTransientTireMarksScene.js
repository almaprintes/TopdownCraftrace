...
export class RaceScene extends CurrentRaceScene {
  ...
  _emitTransientTireMark(nowMs){
    ...
    const minSpeed = raven ? RAVEN_MIN_SPEED : MIN_SPEED;
    if (!Number.isFinite(speed) || speed < minSpeed) { this._lastMarkPoint = null; return; }
    ...
    const ravenContinuousDirt = raven && onTrack;
    if (!ravenContinuousDirt && !strongSlide && !offRoad && !carryingDirt) { this._lastMarkPoint = null; return; }
    ...
    if(prev && prev.markKind===markKind){
      gfx.beginPath();
      gfx.moveTo(prev.leftX, prev.leftY); gfx.lineTo(leftX, leftY);
      gfx.moveTo(prev.rightX, prev.rightY); gfx.lineTo(rightX, rightY);
      gfx.strokePath();
    }else{
      const seg = Math.max(5, Math.min(13, speed * 0.035));
      gfx.beginPath();
      gfx.moveTo(leftX - fx * seg, leftY - fy * seg); gfx.lineTo(leftX, leftY);
      gfx.moveTo(rightX - fx * seg, rightY - fy * seg); gfx.lineTo(rightX, rightY);
      gfx.strokePath();
    }
    try { this.uiCam?.ignore?.(gfx); } catch {}
    this._tireMarks.push(gfx);
    while(this._tireMarks.length > MAX_MARKS){ const old=this._tireMarks.shift(); try{old?.destroy?.();}catch{} }
    this.tweens.add({ targets:gfx, alpha:0, duration:life, ease:'Sine.easeOut', onComplete:()=>{
      const idx=this._tireMarks.indexOf(gfx); if(idx>=0) this._tireMarks.splice(idx,1); try{gfx.destroy();}catch{}
    }});
