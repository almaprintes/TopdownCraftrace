...
export class RaceScene extends CurrentRaceScene {
  ...
  _emitTransientTireMark(nowMs){
    ...
    const fx = Math.cos(heading), fy = Math.sin(heading), lx = -fy, ly = fx;
    ...
    const rearOffset = 27, wheelHalf = 13;
    const rearX = x - fx * rearOffset, rearY = y - fy * rearOffset;
    const leftX = rearX + lx * wheelHalf, leftY = rearY + ly * wheelHalf;
    const rightX = rearX - lx * wheelHalf, rightY = rearY - ly * wheelHalf;
    let color = 0x161616;
    let alpha = 0.24 + clamp01((slip - SLIP_START) / (SLIP_STRONG - SLIP_START)) * 0.28;
    let width = 2.2 + clamp01(slip / 120) * 1.4;
    let life = 1150, markKind = 'rubber';
    if (offRoad) {
      color = onGrass ? 0x756a46 : 0x6e5941;
      alpha = 0.18 + clamp01(speed / 360) * 0.17;
      width = 3.4; life = 820; markKind = onGrass ? 'grass-dirt' : 'off-dirt';
    } else if (carryingDirt) {
      const carry = clamp01((Number(this._dirtCarryUntil || 0) - nowMs) / DIRT_CARRY_MS);
      color = 0xb89562;
      alpha = 0.17 + carry * 0.27;
      width = 4.0;
      life = 1000;
      markKind = 'dirt-carry';
    }
    ...
    if (prev && prev.markKind === markKind) {
      gfx.beginPath(); gfx.moveTo(prev.leftX, prev.leftY); gfx.lineTo(leftX, leftY); gfx.moveTo(prev.rightX, prev.rightY); gfx.lineTo(rightX, rightY); gfx.strokePath();
    } else {
      const seg = Math.max(5, Math.min(13, speed * 0.035));
      gfx.beginPath(); gfx.moveTo(leftX - fx * seg, leftY - fy * seg); gfx.lineTo(leftX, leftY); gfx.moveTo(rightX - fx * seg, rightY - fy * seg); gfx.lineTo(rightX, rightY); gfx.strokePath();
    }
