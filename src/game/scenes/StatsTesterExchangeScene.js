...
export class StatsScene extends CurrentStatsScene{
  ...
  _openTesterExchange(trackId=''){
    ...
    const draw=()=>{const list=card.querySelector('[data-list]');if(!list)return;let fresh=[];try{fresh=loadSharedLaps(selectedTrack)||[];}catch(err){console.warn('[tester-exchange] load failed',err);if(status)status.textContent='No se pudieron leer las vueltas recibidas.';}list.innerHTML=fresh.map((lap,i)=>`<div style="display:grid;grid-template-columns:1fr auto auto;gap:10px;align-items:center;padding:9px;border-top:1px solid #17333f"><div><b>${lap?.pilot?.name||'PILOTO'}</b><div style="font-size:9px;color:#7893a0">${lap?.carId||''} · RECIBIDA</div></div><b>${fmt(lap?.lapMs)}</b><button type="button" data-play="${i}">▶</button></div>`).join('')||'<div style="font-size:10px;color:#7893a0">Sin vueltas recibidas todavía.</div>';list.querySelectorAll('[data-play]').forEach(btn=>btn.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();const lap=fresh[Number(btn.dataset.play)];if(!lap?.replay?.samples?.length)return;root.remove();try{this._showRaceControlReplay({trackId:lap.trackId,selectedLap:{carId:lap.carId,lapMs:lap.lapMs}},{trackKey:lap.trackId,carId:lap.carId,lapMs:lap.lapMs,recordedAt:lap.recordedAt,samples:lap.replay.samples,cameraSamples:lap.replay.cameraSamples||[],viewport:lap.replay.viewport||null});}catch(err){console.error('[tester-exchange] replay failed',err);}},false));};
    card.querySelector('[data-close]')?.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();root.remove();},false);
    card.querySelector('[data-import]')?.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();try{const raw=String(card.querySelector('[data-code]')?.value||'');const pos=raw.indexOf('TDRLAP1:');const imported=decodeSharedLap(pos>=0?raw.slice(pos).trim():raw);if(status){status.textContent=imported.importStatus==='duplicate'?`Esta vuelta ya estaba importada · ${imported.pilot.name} · ${fmt(imported.lapMs)}`:`Vuelta importada correctamente · ${imported.pilot.name} · ${fmt(imported.lapMs)}`;status.style.color=imported.importStatus==='duplicate'?'#ffd68d':'#79f2b0';}draw();}catch(err){if(status){status.textContent=String(err?.message||'Código no válido');status.style.color='#ff9a9a';}}},false);
    draw();
  }
}