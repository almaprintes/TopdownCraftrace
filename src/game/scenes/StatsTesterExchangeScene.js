...
export class StatsScene extends CurrentStatsScene{
  ...
  _openTesterExchange(trackId=''){
    ...
    card.querySelector('[data-import]')?.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();try{const raw=String(card.querySelector('[data-code]')?.value||'');const pos=raw.indexOf('TDRLAP1:');const imported=decodeSharedLap(pos>=0?raw.slice(pos).trim():raw);if(status)status.textContent=`Importada: ${imported.pilot.name} · ${fmt(imported.lapMs)}`;draw();}catch(err){if(status)status.textContent=String(err?.message||'Código no válido');}},false);
    draw();
  }
}