...
const MATERIAL_ASSET={scrap:'chatarra.webp',alloy:'aleacion.webp',rubber:'goma.webp',compound:'compuesto.webp',disc:'disco_metalico.webp',spring:'muelle.webp',gear:'engranaje.webp',ecu:'electronica.webp'};
const BASE=import.meta.env.BASE_URL||'/';
const matSrc=id=>`${BASE}assets/crafting/materials/${MATERIAL_ASSET[id]||''}`;
const itemName=id=>{const key=`items.${id}`,label=t(key);return label===key?(GARAGE_ITEMS[id]?.name||id):label;};
...
function applyAtlanticName(){
  if(TRACK_REGISTRY?.['karting-tenerife']){
    const name=t('track.atlantic');
    TRACK_REGISTRY['karting-tenerife'].name=name;
    TRACK_REGISTRY['karting-tenerife'].meta={...(TRACK_REGISTRY['karting-tenerife'].meta||{}),name};
  }
}
applyAtlanticName();
...
export class RaceScene extends CurrentRaceScene {
  create(data){
    ...
    if(this.track){const name=t('track.atlantic');this.track.name=name;this.track.meta={...(this.track.meta||{}),name};}
    return result;
  }
  ...
  _showSessionRewards(resultRoot=null,onDone=null){
    ...
    </style><div class="tdrfp-veil"><div class="tdrfp-card"><button class="tdrfp-close" data-a="close">×</button><div class="tdrfp-k">${t('race.sessionFinished')}</div><h2>${hasChest?t('race.chestLaps',{laps:chestTier}):t('race.sessionLoot')}</h2><div class="tdrfp-sub">${t('race.sessionLootDesc')}</div>${hasChest?`<div class="tdrfp-chestwrap" data-a="open"><div class="tdrfp-chest ${tierClass}"><div class="glow"></div><div class="base"></div><div class="lid"></div><div class="lock"></div><div class="tier">${t('race.level',{level:chestTier})}</div></div></div><div class="tdrfp-tap">${t('race.tapOpen')}</div>`:''}<div class="tdrfp-summary"><small>${t('race.materialRewards')}</small><b>${t('race.pieces',{count:total})}</b></div><div class="tdrfp-grid">${entries.map(([id,n],i)=>`<div class="tdrfp-item ${n<=0?'zero':''}" data-r="${i}"><div class="tdrfp-asset"><img src="${matSrc(id)}" alt=""></div><div class="tdrfp-copy"><div class="tdrfp-name">${itemName(id)}</div><div class="tdrfp-q">×${n}</div></div></div>`).join('')}</div><div class="tdrfp-meta"><span class="tdrfp-chip">${t('race.rewardedLaps',{laps:rewardedLaps})}</span>${Number(summary?.bonusLaps||0)?`<span class="tdrfp-chip">${t('race.bonus',{laps:Number(summary.bonusLaps)})}</span>`:''}${hasChest?`<span class="tdrfp-chip">${t('race.chest',{laps:chestTier})}</span>`:''}</div><button class="tdrfp-next" data-a="next">${resultRoot?t('race.viewResults'):t('race.viewReport')}</button></div></div>`;
  ...
  _showSurvivalResults(){
    ...
    const names=[...root.querySelectorAll('.tdrsurv-loot-name')];names.forEach((node,i)=>{const id=entries[i]?.[0];if(id)node.textContent=itemName(id);});
  ...
  _showSurvivalSessionInfo(resultRoot){
    ...
    const rows=laps.length?`<div class="tdrsi-f1"><div class="tdrsi-head"><b>${t('common.lap')}</b><b>S1</b><b>S2</b><b>S3</b><b>${t('common.total')}</b><b></b></div>${laps.map(l=>{const s=Array.isArray(l.sectors)?l.sectors:[null,null,null],isBest=Number(l.lapMs)===best;return`<div class="tdrsi-row${isBest?' best':''}"><b>V${l.n}</b>${cell(s[0],0)}${cell(s[1],1)}${cell(s[2],2)}<strong>${fmtLap(l.lapMs)}</strong><i>${isBest?t('common.best'):''}</i></div>`;}).join('')}</div><div class="tdrsi-legend"><span><u class="green"></u>${t('race.bestSessionShort')}</span><span><u class="purple"></u>${t('race.sectorRecordShort')}</span></div>`:`<div class="tdrsi-empty">${t('race.noTimedLaps')}</div>`;
    const trackName=String(this.track?.meta?.name||this.track?.name||this.trackId||this.trackKey||t('track.generic'));
    ...
    </style><div class="tdrsi-veil"><div class="tdrsi-card"><div class="tdrsi-kicker">${t('race.survivalSessionInfo')}</div><h2>${trackName}</h2><div class="tdrsi-sub">${this.carId||t('car.generic')} · ${t('race.result')} ${this._survivalWon?t('race.champion'):t('race.eliminated')}</div><div class="tdrsi-grid"><div class="tdrsi-stat"><small>${t('race.rounds')}</small><b>${this._survivalRound}/5</b></div><div class="tdrsi-stat"><small>${t('race.best')}</small><b>${fmtLap(best)}</b></div><div class="tdrsi-stat"><small>${t('race.average')}</small><b>${fmtLap(avg)}</b></div><div class="tdrsi-stat"><small>${t('race.laps')}</small><b>${laps.length}</b></div><div class="tdrsi-stat"><small>${t('race.worst')}</small><b>${fmtLap(worst)}</b></div><div class="tdrsi-stat"><small>${t('race.startingCars')}</small><b>6</b></div></div><h3>${t('race.lapsSectors').toUpperCase()}</h3>${rows}<button class="tdrsi-back" data-a="back">${t('race.backResult')}</button></div></div>`;
    if(resultRoot)resultRoot.style.display='none';
