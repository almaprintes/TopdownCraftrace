...
export class RaceScene extends CurrentRaceScene {
  create(data){
    ...
    for(const key of ['raceInfoHud','competitionHud','minimapSportFrame','_perfDiagText','_renderPerfText','_isoText','_diagText','_touchDbg','_dbgText','_lapBreakdownText','devBox','devTitle','devInfo','devBtnMap','devTuneBtn','_simpleRaceTop','_simpleRaceBottom']){safeDestroy(this[key]);this[key]=null;}
    ...
    this._purgeRaceDiagnostics=()=>{for(const obj of [...(this.children?.list||[])]){if(obj?.type!=='Text')continue;const s=String(obj.text||'');if(/\bOBJ\s|\bBUF\s|WEBGL\/|\bDPR\s|\bMAXT\s|GPU\sApple|DB\s\d/i.test(s))safeDestroy(obj);}};
    this._purgeRaceDiagnostics();this._raceDiagAccum=0;
  ...
  update(time,delta){const result=super.update(time,delta);if(this._iosFixedZoom){try{this._zoomCurrent=this._iosFixedZoom;this.cameras?.main?.setZoom?.(this._iosFixedZoom);}catch{}}this._raceDiagAccum=(this._raceDiagAccum||0)+Math.max(0,Number(delta)||0);if(this._raceDiagAccum>500){this._raceDiagAccum=0;this._purgeRaceDiagnostics?.();}this._updateSimpleRaceHud?.(delta);return result;}
}
