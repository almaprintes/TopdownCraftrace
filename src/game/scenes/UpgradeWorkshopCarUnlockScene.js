...
function touchMobile(){
  try{return /Android|iPhone|iPad|iPod/i.test(String(navigator?.userAgent||''))||Number(navigator?.maxTouchPoints||0)>1;}catch{return false;}
}
...
export class UpgradeShopScene extends CurrentWorkshop {
  ...
  _nativeWorkshopDomEnabled(){
    const h=Number(this.scale?.height||window.innerHeight||0);
  ...
  _compactCarPanel(A,r,compact){
    ...
    if(!geo)return;
    const cover=(x,glyph)=>{
      A(this.add.rectangle(x,geo.rowY,geo.buttonW,geo.buttonH,0x081525,1).setStrokeStyle(1,0x405262,.35));
      A(this.add.text(x,geo.rowY,glyph,{fontFamily:'system-ui',fontSize:compact?'18px':'22px',fontStyle:'900',color:'#536777'}).setOrigin(.5));
    };
