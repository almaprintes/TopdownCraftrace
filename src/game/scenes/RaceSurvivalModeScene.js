...
export class RaceScene extends CurrentRaceScene{
  ...
  _showSurvivalResults(){
    ...
    const cpuReplayReady=Boolean(this._survivalCpuReplay?.samples?.length>4);
    const cpuResultPanel=this._survivalPlannerBot?`
      <section class="tdrsurv-cpu">
        ...
        <button class="tdrsurv-cpu-replay" data-a="cpuReplay" ${cpuReplayReady?'':'disabled'}>▶ VER MEJOR VUELTA DE CPU1</button>
      </section>`:'';
    ...
    root.querySelector('[data-a="cpuReplay"]')?.addEventListener('click',()=>this._startSurvivalCpuReplay?.(root));
