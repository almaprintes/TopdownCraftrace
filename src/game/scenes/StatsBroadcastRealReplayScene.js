...
export class StatsScene extends ReplayStatsScene{
  ...
  _bindReplayModalControls(ghost){
    ...
    root.querySelector('[data-br-play]')?.addEventListener('click',()=>{const s=this._raceReplayState();if(!s)return;if(s.finished){s.elapsed=0;s.finished=false;s.playing=true;try{race()?._applyStatsReplayFrame?.(0);}catch{}}else s.playing=!s.playing;});
