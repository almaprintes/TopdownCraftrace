...
export class StatsScene extends ReplayStatsScene{
  ...
  _installBroadcastStyles(){
    ...
    s.textContent=`
      ...
      @media(max-height:430px){.br-native-monitor{min-height:0!important;height:auto!important}.br-native-screen{height:252px}.br-native-monitor .br-monitor-head{height:44px}.br-replay-modal{padding:5px}.br-replay-modal__screen{height:300px;min-height:0;padding-bottom:0}.br-replay-modal__head{height:40px}.br-tel-bottom{bottom:0;height:50px}.br-replay-controls{height:52px;flex:0 0 52px;grid-template-columns:58px 58px 132px 58px minmax(100px,1fr) 78px 58px 118px}.br-replay-controls button{height:32px}.br-replay-controls .br-transport{flex-basis:132px!important;width:132px!important;min-width:132px!important;max-width:132px!important}.br-tel-card{padding:6px 8px}.br-tel-row{padding-top:2px;margin-top:2px}}
    `;
  ...
  _showRaceControlReplayModal(record,ghost,label='REPLAY'){
    ...
    modal.innerHTML=`<section class="br-replay-modal__panel" role="dialog" aria-modal="true" aria-label="Race Control replay"><div class="br-replay-modal__head"><div><div class="br-label">RACE CONTROL // ${esc(label)}</div><strong>${esc(trackId).toUpperCase()}</strong><small>${esc(carId).toUpperCase()} · ${fmt(ghost.lapMs)}</small></div><button type="button" class="br-replay-modal__close" data-br-modal-close>CERRAR ✕</button></div><div class="br-replay-modal__screen" data-br-native-replay-screen="1"><div class="br-native-loading" data-br-native-loading>CARGANDO REPLAY REAL…</div><div class="br-replay-telemetry" data-br-telemetry><aside class="br-tel-left"><div class="br-tel-card"><div class="br-tel-label">CRONO / VUELTA</div><div class="br-tel-big" data-tel-time>0:00.000</div><div class="br-tel-row"><span>TIEMPO VUELTA</span><b>${fmt(ghost.lapMs)}</b></div><div class="br-tel-row"><span>RESTANTE</span><b data-tel-remaining>—</b></div><div class="br-tel-row"><span>PROGRESO</span><b data-tel-progress>0%</b></div><div class="br-tel-row"><span>SECTOR</span><b data-tel-sector>S1</b></div></div><div class="br-tel-card"><div class="br-tel-label">PILOTO / MÁQUINA</div><div class="br-tel-car">${esc(carId).toUpperCase()}</div><div class="br-tel-row"><span>CIRCUITO</span><b>${esc(trackId).toUpperCase()}</b></div><div class="br-tel-row"><span>FUENTE</span><b>${esc(label)}</b></div><div class="br-tel-row"><span>MUESTRAS</span><b>${ghost.samples.length}</b></div><div class="br-tel-row"><span>FRECUENCIA</span><b data-tel-hz>—</b></div></div></aside><aside class="br-tel-right"><div class="br-tel-card"><div class="br-tel-label">DINÁMICA</div><div class="br-tel-speed"><span data-tel-speed>0</span> <small>km/h</small></div><div class="br-tel-bar"><i data-tel-speedbar></i></div><div class="br-tel-row"><span>V. MÁX</span><b data-tel-vmax>0 km/h</b></div><div class="br-tel-row"><span>V. MEDIA</span><b data-tel-vavg>0 km/h</b></div><div class="br-tel-row"><span>ACEL.</span><b data-tel-accel>0 km/h/s</b></div><div class="br-tel-row"><span>GIRO</span><b data-tel-yaw>0°/s</b></div></div><div class="br-tel-card"><div class="br-tel-label">TRAZADA</div><div class="br-tel-row"><span>RUMBO</span><b data-tel-heading>0°</b></div><div class="br-tel-row"><span>CURVA</span><b data-tel-turn>RECTA</b></div><div class="br-tel-row"><span>RECORRIDO</span><b data-tel-path>0%</b></div><div class="br-tel-row"><span>RESTANTE</span><b data-tel-pathleft>100%</b></div><div class="br-tel-row"><span>MUESTRA</span><b data-tel-pos>0 / 0</b></div></div></aside></div><div class="br-replay-controls" data-br-controls><button type="button" data-br-start>↶ 0</button><button type="button" data-br-prev>◀│</button><button type="button" class="br-transport" data-br-play aria-label="Pausar"><span class="br-transport__icon" data-br-play-icon>⏸</span><span class="br-transport__label" data-br-play-label>PAUSA</span></button><button type="button" data-br-next>│▶</button><input data-br-seek type="range" min="0" max="${Math.max(1,Math.round(Number(ghost.lapMs)||1))}" step="1" value="0" aria-label="Posición del replay"><span class="br-ctl-time" data-br-ctl-time>0:00.000</span><button type="button" data-br-speed>1×</button><button type="button" data-br-analysis>TRAZADA ✓</button></div></section>`;
    this._root.appendChild(modal);
    const panel=modal.querySelector('.br-replay-modal__panel'),controls=modal.querySelector('[data-br-controls]');
    if(panel&&controls&&controls.parentElement!==panel)panel.appendChild(controls);
    ...
    this._bindReplayModalControls(ghost);
  }

  _raceReplayState(){try{return this.scene?.get?.('race')?._tdrStatsReplay||null;}catch{return null;}}
  _bindReplayModalControls(ghost){
    const root=this._root?.querySelector('[data-br-replay-modal]');if(!root)return;
    const race=()=>{try{return this.scene?.get?.('race')||null;}catch{return null;}};
    const seek=t=>{const s=race();try{s?._seekStatsReplay?.(Math.max(0,Math.min(Number(ghost?.lapMs)||0,Number(t)||0)),true);}catch{}};
    root.querySelector('[data-br-start]')?.addEventListener('click',()=>seek(0));
    root.querySelector('[data-br-prev]')?.addEventListener('click',()=>{try{race()?._stepStatsReplaySample?.(-1);}catch{}});
    root.querySelector('[data-br-next]')?.addEventListener('click',()=>{try{race()?._stepStatsReplaySample?.(1);}catch{}});
    root.querySelector('[data-br-play]')?.addEventListener('click',()=>{const s=this._raceReplayState();if(!s)return;if(s.finished||Number(s.elapsed)>=Number(ghost?.lapMs||0)-1){try{race()?._seekStatsReplay?.(0,true);}catch{}s.elapsed=0;s.finished=false;s.playing=true;try{race()?._applyStatsReplayFrame?.(0);}catch{}}else{s.playing=!s.playing;}const b=root.querySelector('[data-br-play]'),bi=b?.querySelector('[data-br-play-icon]'),bl=b?.querySelector('[data-br-play-label]');if(b){if(bi)bi.textContent=s.playing?'⏸':'▶';if(bl)bl.textContent=s.playing?'PAUSA':'REPRODUCIR';b.title=s.playing?'Pausar':'Reproducir';b.setAttribute('aria-label',b.title);}});
    root.querySelector('[data-br-speed]')?.addEventListener('click',e=>{const s=this._raceReplayState();if(!s)return;const speeds=[.25,.5,1,2],idx=Math.max(0,speeds.indexOf(Number(s.speed)||1));s.speed=speeds[(idx+1)%speeds.length];e.currentTarget.textContent=s.speed+'×';});
    root.querySelector('[data-br-analysis]')?.addEventListener('click',e=>{const s=race();if(!s)return;s._tdrReplayAnalysisEnabled=!s._tdrReplayAnalysisEnabled;e.currentTarget.textContent=s._tdrReplayAnalysisEnabled?'TRAZADA ✓':'TRAZADA';const a=root.querySelector('[data-tel-analysis]');if(a)a.textContent=s._tdrReplayAnalysisEnabled?'ANÁLISIS ON':'ANÁLISIS OFF';try{s._copyEmbeddedReplayFrame?.();}catch{}});
    root.querySelector('[data-br-seek]')?.addEventListener('input',e=>seek(e.currentTarget.value));
  }
