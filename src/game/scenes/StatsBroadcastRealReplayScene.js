...
function drawRibbon(ctx,ribbon,cam,scale,w,h){const left=ribbon?.left||[],right=ribbon?.right||[];if(left.length<2||right.length<2)return;
  ctx.beginPath();left.forEach((p,i)=>{const q=worldToScreen(p[0],p[1],cam,scale,w,h);i?ctx.lineTo(q.x,q.y):ctx.moveTo(q.x,q.y);});for(let i=right.length-1;i>=0;i--){const q=worldToScreen(right[i][0],right[i][1],cam,scale,w,h);ctx.lineTo(q.x,q.y);}ctx.closePath();ctx.fillStyle='#575b5d';ctx.fill();
  for(const edge of [left,right]){path(ctx,edge,cam,scale,w,h);ctx.strokeStyle='#f4f1e8';ctx.lineWidth=Math.max(2,3.2*scale);ctx.lineJoin='round';ctx.lineCap='round';ctx.stroke();}
}
...
function drawFinish(ctx,finish,cam,scale,w,h){if(!finish?.a||!finish?.b)return;const a=worldToScreen(finish.a.x,finish.a.y,cam,scale,w,h),b=worldToScreen(finish.b.x,finish.b.y,cam,scale,w,h);const dx=b.x-a.x,dy=b.y-a.y,len=Math.hypot(dx,dy)||1,steps=10;for(let i=0;i<steps;i++){const t0=i/steps,t1=(i+1)/steps;ctx.beginPath();ctx.moveTo(a.x+dx*t0,a.y+dy*t0);ctx.lineTo(a.x+dx*t1,a.y+dy*t1);ctx.strokeStyle=i%2?'#101316':'#f6f6f3';ctx.lineWidth=Math.max(5,10*scale);ctx.stroke();}}
...
`;this._root.appendChild(s);}
  async _showRaceControlReplay(record,ghost){
    ...
    // Fixed-world television camera. It pans only; the circuit never rotates.
    // The scale is tied to the road width so car/road proportions stay believable across circuits.
    const scale=Math.max(.64,Math.min(.92,82/Math.max(70,roadWidth)));
    let cam={x:Number(ghost.samples[0]?.x)||0,y:Number(ghost.samples[0]?.y)||0};
    const render=t=>{const car=pointAt(ghost.samples,t),w=canvas.width,h=canvas.height;cam.x+=(car.x-cam.x)*.13;cam.y+=(car.y-cam.y)*.13;ctx.clearRect(0,0,w,h);ctx.fillStyle='#183d24';ctx.fillRect(0,0,w,h);
      drawRibbon(ctx,ribbon,cam,scale,w,h);drawKerbs(ctx,ribbon,cam,scale,w,h);drawFinish(ctx,track.finishLine||track.finish,cam,scale,w,h);
      for(const item of envItems){const im=envMap.get(`${BASE}assets/${item.path}`);if(!im)continue;const q=worldToScreen(Number(item.x),Number(item.y),cam,scale,w,h),targetW=(Number(item.displayWidth)||im.naturalWidth||40)*scale;if(q.x<-220||q.x>w+220||q.y<-220||q.y>h+220)continue;const ratio=(im.naturalHeight||1)/(im.naturalWidth||1),targetH=targetW*ratio;ctx.save();ctx.translate(q.x,q.y);ctx.rotate(Number(item.rotation)||0);ctx.scale(item.flipX?-1:1,item.flipY?-1:1);ctx.drawImage(im,-targetW/2,-targetH/2,targetW,targetH);ctx.restore();}
      const cp=worldToScreen(car.x,car.y,cam,scale,w,h);if(carImg){const ratio=(carImg.naturalHeight||1)/(carImg.naturalWidth||1);const carWorldW=roadWidth*.29,cw=carWorldW*scale,ch=cw*ratio;ctx.save();ctx.translate(cp.x,cp.y);ctx.rotate(car.r+Math.PI/2);ctx.shadowColor='rgba(0,0,0,.48)';ctx.shadowBlur=6;ctx.drawImage(carImg,-cw/2,-ch/2,cw,ch);ctx.restore();}else{ctx.save();ctx.translate(cp.x,cp.y);ctx.rotate(car.r);ctx.fillStyle='#61eafa';ctx.fillRect(-12,-6,24,12);ctx.restore();}}
