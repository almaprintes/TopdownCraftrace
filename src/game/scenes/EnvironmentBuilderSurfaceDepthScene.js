...
function drawSurface(g,s){
  ...
  const width=Math.max(2,Number(s.width)||120),half=width/2,steps=Math.max(32,Math.min(260,Math.ceil(pathLength(s,120)/24))),left=[],right=[];
  for(let i=0;i<=steps;i++){
    const t=i/steps,p=pathPoint(s,t),v=pathTangent(s,t),len=Math.hypot(v.x,v.y)||1,nx=-v.y/len,ny=v.x/len;
...
export class EnvironmentBuilderScene extends CurrentEnvironmentBuilderScene {
  ...
  _addSurface(a,b,width,physics,data=null){
    ...
    if(s&&this._surfaces.length>before){s.trackLayer=data?.trackLayer==='over'?'over':'under';this._redrawSurfaces();}
  }
  _selectSurface(s){if(s&&!s.trackLayer)s.trackLayer='under';super._selectSurface(s);this._refreshSurfaceDepthButton();}
  _select(obj){super._select(obj);this._refreshSurfaceDepthButton();}
  _toggleSelectedSurfaceDepth(){
    const s=this._selectedSurface;if(!s){this._flash?.('SELECCIONA UN TERRENO');return;}
    s.trackLayer=s.trackLayer==='over'?'under':'over';this._redrawSurfaces();this._refreshSurfaceDepthButton();this._flash?.(s.trackLayer==='over'?'TERRENO SOBRE PISTA':'TERRENO BAJO PISTA');
  }
  _refreshSurfaceDepthButton(){const t=this._surfaceDepthBtn?.t;if(!t)return;if(!this._selectedSurface){t.setText('—');return;}t.setText(this._selectedSurface.trackLayer==='over'?'SOBRE':'BAJO');}
  _redrawSurfaces(){
    const under=this._surfaceG,over=this._surfaceOverG;under?.clear?.();over?.clear?.();
    for(const s of this._surfaces||[]){if(!s.trackLayer)s.trackLayer='under';drawSurface(s.trackLayer==='over'?(over||under):under,s);}
  }
  _applyProject(p){super._applyProject(p);for(const s of this._surfaces||[])if(!s.trackLayer)s.trackLayer='under';this._redrawSurfaces();this._refreshSurfaceDepthButton();}
}
