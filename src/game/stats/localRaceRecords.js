...
function buildTrack(trackId){const laps=historyForTrack(trackId).sort((a,b)=>a.lapMs-b.lapMs||b.timestamp-a.timestamp);const pb=officialPb(trackId);const matchingLap=pb?laps.find(l=>Math.abs(Number(l.lapMs)-Number(pb.lapMs))<=1):null;const best=pb||laps[0]||null;const bestCarId=text(best?.carId)||text(matchingLap?.carId)||text(laps[0]?.carId)||null;const recordedAt=Number(best?.recordedAt||best?.timestamp||matchingLap?.timestamp||laps[0]?.timestamp||0)||0;const progression=[];let running=Infinity;for(const lap of [...laps].sort((a,b)=>a.timestamp-b.timestamp)){if(lap.lapMs<running){running=lap.lapMs;progression.push({lapMs:lap.lapMs,timestamp:lap.timestamp,carId:lap.carId});}}
return{trackId,bestLapMs:best?.lapMs||null,bestCarId,recordedAt,ghostKey:best?.ghostKey||matchingLap?.replayKey||null,validLaps:laps.length,topLaps:laps.slice(0,TOP_LIMIT),progression:progression.slice(-50),verificationStatus:'local'};}
