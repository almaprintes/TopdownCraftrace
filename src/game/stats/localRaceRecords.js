...
function normalizeLap(trackId,row,index){const lapMs=positive(row?.lapMs??row?.ms??row?.time);if(!lapMs||row?.valid===false||row?.invalid===true)return null;return{id:lapId(trackId,row,index),trackId,carId:text(row?.carId)||null,lapMs:Math.round(lapMs),sectorsMs:sectors(row),timestamp:Math.max(0,Number(row?.t||row?.timestamp||0)),valid:true,verificationStatus:'local'};}
...
function discoverTrackIds(){const ids=new Set();try{for(let i=0;i<localStorage.length;i++){const key=localStorage.key(i)||'';if(key.startsWith(TT_HISTORY_PREFIX))ids.add(key.slice(TT_HISTORY_PREFIX.length));if(key.startsWith(PB_PREFIX))ids.add(key.slice(PB_PREFIX.length));}}catch{}return[...ids].filter(Boolean);}
