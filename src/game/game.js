...
const LAZY_SCENES={
  ...
  race:{
    load:async()=>{
      const [{RaceScene},{installExactRuntimeBeautyPass},{installRaceEnvironmentRuntime},{installRaceLoadingExperience},{installLiveDeltaReferenceRuntime},{installLapGhostRuntime}]=await Promise.all([
        ...
        import('./ui/liveDeltaReferenceRuntime.js'),
        import('./ui/lapGhostRuntime.js')
      ]);
      installRaceEnvironmentRuntime(RaceScene);
      installExactRuntimeBeautyPass(RaceScene);
      installRaceLoadingExperience(RaceScene);
      installLiveDeltaReferenceRuntime(RaceScene);
      installLapGhostRuntime(RaceScene);
      return {RaceScene};
    },
  ...
  EnvironmentBuilderScene:{load:()=>import('./scenes/EnvironmentBuilderMaterialStudioScene.js'),exportName:'EnvironmentBuilderScene',admin:true}
};
...
function scheduleSceneWarmup(){
  ...
  const run=async()=>{
    await new Promise(resolve=>{if(typeof window.requestIdleCallback==='function')window.requestIdleCallback(()=>resolve(),{timeout:2500});else setTimeout(resolve,1800);});
    for(const [key] of warm){if(cancelled||document.hidden)break;await ensureLazyScene(key);await new Promise(resolve=>{if(typeof window.requestIdleCallback==='function')window.requestIdleCallback(()=>resolve(),{timeout:700});else setTimeout(resolve,180);});}
    cancel();
  };
  window.addEventListener('pointerdown',cancel,true);window.addEventListener('keydown',cancel,true);window.addEventListener('tdr:bootready',()=>{run().catch(()=>cancel());},{once:true});
}
function videoPrefs(){try{const s=JSON.parse(localStorage.getItem('tdr2:settings')||'{}'),v=s?.video||{};return{quality:String(v.quality||'high'),targetFps:[30,45,60].includes(Number(v.targetFps))?Number(v.targetFps):60,antialias:typeof v.antialias==='boolean'?v.antialias:String(v.quality||'high')!=='low'};}catch{return{quality:'high',targetFps:60,antialias:true};}}
function isLegacyIOSPhone(){try{if(!isIOSDevice())return false;const maxSide=Math.max(Number(screen?.width||0),Number(screen?.height||0));const phoneLike=maxSide<=900;const olderPhoneClass=phoneLike&&maxSide<=844;const manualSafe=localStorage.getItem('tdr2:forceIosSafeMode')==='1';const autoSafeUntil=Number(localStorage.getItem('tdr2:autoIosSafeModeUntil')||0);const autoSafe=Number.isFinite(autoSafeUntil)&&autoSafeUntil>Date.now();return olderPhoneClass||manualSafe||autoSafe;}catch{return false;}}
...
function installSafeTextureGuard(){const classes=[Phaser.GameObjects?.Image,Phaser.GameObjects?.Sprite,Phaser.GameObjects?.TileSprite].filter(Boolean);for(const Klass of classes){const proto=Klass?.prototype;if(!proto||proto.__tdrSafeTextureGuard||typeof proto.setTexture!=='function')continue;const original=proto.setTexture;proto.setTexture=function(...args){if(!this?.scene?.sys?.textures)return this;return original.apply(this,args);};proto.__tdrSafeTextureGuard=true;}}
export function createGame(parentId='app'){
  ...
  const vp=videoPrefs(),iosDevice=isIOSDevice(),safeMode=isLegacyIOSPhone();try{window.__tdrIosSafeMode=safeMode;}catch{}const antialias=iosDevice?false:!!vp.antialias,targetFps=safeMode?30:vp.targetFps;const batchSize=iosDevice?1024:4096;const forceSetTimeOut=iosDevice&&!forceRafLoop();
  ...
  installHtmlTextRuntime(game);installLazySceneNavigation(game);scheduleSceneWarmup();try{window.__tdrEnsureScene=ensureLazyScene;}catch{}try{const canvas=game.canvas;if(canvas?.style){canvas.style.imageRendering='auto';canvas.style.webkitFontSmoothing='antialiased';canvas.style.textRendering='optimizeLegibility';}}catch(_){}installOrientationViewportSettle(game);installRuntimeCrashDiagnostics(game);installMenuMusic(game);return game;
}
