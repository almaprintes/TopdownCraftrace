...
import { RaceScene } from './scenes/RaceDuelLapReplayScene.js';
import { UpgradeShopScene } from './scenes/UpgradeWorkshopCoinAssetScene.js';
...
export function createGame(parentId='app'){installCleanTextFactory();const vp=videoPrefs();const dpr=window.devicePixelRatio||1;const requestedScale=vp.renderScale==='eco'?1:vp.renderScale==='sharp'?2:1.5;const requestedQuality=vp.quality==='low'?1:vp.quality==='medium'?1.5:2;const requested=Math.min(requestedScale,requestedQuality);const ios=isIOSDevice();const resolution=ios?Math.min(dpr,Math.max(1,requested),1.5):Math.min(dpr,Math.max(2,requested));const game=new Phaser.Game({type:Phaser.AUTO,parent:parentId,backgroundColor:'#0b1020',resolution,fps:{target:vp.targetFps,min:20},scene:[BootScene,MenuScene,MenuAliasScene,GarageScene,SettingsScene,GarageDetailScene,RaceScene,AdminHubScene,UpgradeShopScene,CarEditorScene,TrackGarageScene,TrackStudioScene,EnvironmentBuilderScene,TrackEditorScene],dom:{createContainer:true},scale:{mode:Phaser.Scale.RESIZE,autoCenter:Phaser.Scale.CENTER_BOTH},physics:{default:'arcade',arcade:{debug:false}},render:{pixelArt:false,antialias:true,antialiasGL:true,roundPixels:false}});try{const canvas=game.canvas;if(canvas?.style){canvas.style.imageRendering='auto';canvas.style.webkitFontSmoothing='antialiased';canvas.style.textRendering='optimizeLegibility';}}catch(_){}installRuntimeCrashDiagnostics(game);installMenuMusic(game);return game;}