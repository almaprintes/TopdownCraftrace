# Asset licenses

Registro de procedencia, autoría y licencia de assets externos usados por Top Down RACE / TopdownCraftrace.

## domasx2 / OpenGameArt — banco RPM de Spark y Gripline

Los seis loops de motor usados por HÉLIX Spark y AVENIR Gripline proceden de **Racing Car Engine Sound Loops**, publicado por `domasx2` en OpenGameArt bajo licencia **CC0 1.0 Universal**.

- Fuente: https://opengameart.org/content/racing-car-engine-sound-loops
- Autor: `domasx2`
- Licencia: https://creativecommons.org/publicdomain/zero/1.0/
- Rutas locales: `public/assets/audio/engine/spark/loop_0.wav` a `loop_5_0.wav`
- Perfiles runtime: modelo específico de Spark y `ENGINE_AUDIO_PROFILES.AVENIR_GRIPLINE`
- Hashes individuales: `public/assets/audio/engine/spark/README.md`

Ambos coches comparten los WAV auditados, pero usan curvas de RPM, tono y mezcla diferentes. No existe descarga remota durante el juego.

## buntine/CarEngines — motor del perfil JEEP

La muestra aprobada para el perfil de audio `JEEP`, asignado al HÉLIX Vortex, procede del repositorio público `buntine/CarEngines`, publicado bajo licencia **CC0 1.0**.

- Repositorio y licencia: https://github.com/buntine/CarEngines
- Archivo de origen: `sounds/engine.wav`
- Ruta local: `public/assets/audio/engine/jeep/engine.wav`
- SHA-256: `8299d3d595fe4c6b2ed9cf73f7d4ead854e7f97470e59178e8ff9a0b15a41645`
- Perfil runtime: `src/game/audio/EngineAudioProfiles.js` → `JEEP`

La copia local conserva exactamente la muestra que utilizaba el Vortex y evita depender de GitHub Raw durante el juego.

## Poly Haven — superficies de terreno

Poly Haven publica sus assets bajo licencia **CC0**. Se permite su uso, modificación y distribución, incluido el uso comercial, sin obligación de atribución.

Licencia oficial: https://polyhaven.com/license

### Autor: Amal Kumar

Las siguientes texturas usadas en el proyecto proceden de assets de Poly Haven creados por **Amal Kumar**:

- **OFF / terreno rocoso** — textura de terreno rocoso utilizada para la superficie `off`.
- **Hierba / terreno natural** — textura verde con roca utilizada para la superficie `grass`.

Ambas quedan registradas como assets **CC0 de Poly Haven**, autor **Amal Kumar**.

### Perfil y contacto del autor

Información pública verificada:

- Nombre: **Amal Kumar**
- Actividad: **3D Artist — Photogrammetry, Game Art**
- Ubicación indicada en su perfil profesional: **Ranchi, India**
- ArtStation: https://amalbubble.artstation.com/
- Portfolio ArtStation: https://amalbubble.artstation.com/projects
- Resume / contacto público: https://amalbubble.artstation.com/resume
- Email profesional publicado en su resume de ArtStation: **amalbubble@gmail.com**
- Perfil de assets en Poly Haven: https://polyhaven.com/all?a=Amal%20Kumar

Poly Haven identifica a Amal Kumar como colaborador de **Textures**.

### Apoyo / donación futura

Poly Haven se financia en buena parte mediante donaciones y dispone de Patreon oficial:

- Patreon de Poly Haven: https://www.patreon.com/polyhaven
- Información oficial de apoyo y contacto: https://polyhaven.com/about-contact

No se ha confirmado un Patreon personal independiente de Amal Kumar. Si Top Down RACE obtiene ingresos y se decide apoyar económicamente al autor de estas texturas, comprobar primero si Amal mantiene entonces un canal de apoyo personal público. En ausencia de uno, **Poly Haven Patreon** es un canal oficial válido para contribuir al proyecto que distribuye estos assets CC0.

### Implementación actual

- `grass` → `public/assets/materials/grass/rocky_terrain_02_diff_2k.jpg`
- `off` → `public/assets/materials/offroad/rocky_terrain_diff_2k.jpg`

La antigua versión 4K del OFF puede conservarse como fuente de alta resolución, pero el runtime usa la versión 2K más ligera.

## Poly Haven — asfalto

### Autor: Dimitrios Savva

La textura **Clean Asphalt** usada actualmente para la superficie `asphalt` procede de Poly Haven y aparece publicada bajo licencia **CC0**.

- Autor: **Dimitrios Savva**
- Asset: **Clean Asphalt**
- Categoría mostrada por Poly Haven: **Asphalt & Bitumen → Asphalt → Fresh Asphalt**
- Resolución usada en runtime: **2K**
- Ruta actual: `public/assets/materials/asphalt-pbr/clean_asphalt_diff_2k.jpg`
- Licencia: **CC0**
- Licencia oficial de Poly Haven: https://polyhaven.com/license
- Página de autor/assets en Poly Haven: https://polyhaven.com/all?a=Dimitrios%20Savva

Se conserva esta autoría aunque CC0 no obliga a atribuir, para mantener trazabilidad y poder acreditar o apoyar al creador en el futuro.

## Criterio del proyecto

Aunque CC0 no exige atribución, conservamos este registro para mantener trazabilidad sobre la procedencia de los assets y facilitar futuras revisiones antes de publicación.

También conservamos los datos públicos de los autores para poder acreditarlos voluntariamente, contactar con ellos o apoyarlos económicamente en el futuro si el proyecto genera ingresos.
